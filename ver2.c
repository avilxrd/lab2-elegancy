...
void move_do_deposito(Carta pilhas[NUM_PILHAS][MAX_CARTAS], Carta descarte[52], int pilhaDestino, int *tamanho_pilha){
    ...
    for(int i = 0; i < 52; i++){
        if(descarte[i].valor != 0){
            ...
            carta_sera_movida.visivel = true;
        }
    }
    ...
    // PARTE DAS PILHAS FINAIS
    else{
        Carta *pilha_final;
        switch (carta_sera_movida.naipe) {
            case 0:
                pilha_final = pilhaEspadas;
                break;
            case 1:
        ...
        if (carta_sera_movida.valor == proximo_valor) {
            for (int i = 0; i < MAX_CARTAS; i++) {
                if (pilha_final[i].valor == 0) {
                    pilha_final[i] = carta_sera_movida;
                    break;
                }
            }
            ...
            muda_cor(32);
            printf("Carta %d%s movida para a pilha final do naipe %s.\n", carta_sera_movida.valor, lista_de_naipes[carta_sera_movida.naipe], lista_de_naipes[carta_sera_movida.naipe]);
            muda_cor(0);
        } else {
            muda_cor(31);
            printf("Movimento inválido! O valor da carta deve ser %d.\n", proximo_valor);
...
int main(){
    ...
    do{
        ...
        else if (instrucao == 9 && primeira_carta >= 0){
            int pilhaDestino;
